def epoching(args, data_part, eeg_type, seed):
    ...
    chan_order = ['O1', 'Oz', 'O2', 'PO7', 'PO3', 'POz','PO4', 'PO8', 'P7', 'P5',
    ...
    for s in range(args.n_ses):
        ...
        #     data_part+'.npy')
        sub = "sub-" + format(args.sub, "02")
        ses = "ses-" + format(s+1, '02')
        eeg_dir = f"/path/to/THINGS-EEG2/raw_eeg_data/{sub}/{ses}/{sub}_{ses}_task-{data_part}_{eeg_type}.npy"
...
def save_prepr(args, whitened_test, whitened_train, img_conditions_train,
	...
	del merged_test
	# Saving directories
	save_dir = os.path.join(args.project_dir, "preprocessed_eeg_data_new",
		 'sub-'+format(args.sub,'02'))
	file_name_test = f'preprocessed_eeg_test_{eeg_type}.npy'
	file_name_train = f'preprocessed_eeg_training_{eeg_type}.npy'
